...
CHARTS_TEMPLATE = go.layout.Template(
    layout=dict(
        ...
        legend=dict(orientation="h", title_text="", x=0, y=1.2),
    )
)
            ...
            [
                html.Div(
                    [
                        ...
                        html.H2(
                            id="total-turbine",
                            style={"margin-top": "6px", "color": "black"},
                        ),
                    ]
                ),
                html.Div(
...
graph_card1 = dbc.Card(
    [
        dbc.CardBody(dcc.Graph(id="capacity-distribution", responsive=True)),
    ],
...
graph_card2 = dbc.Card(
    [
        dbc.CardBody(dcc.Graph(id="power-by-years", responsive=True)),
    ],
    style={"width": "100%", "margin": "20px"},
)
...
content = html.Div(
    [
        dbc.Row(
            [dbc.Col(graph_card1, width=6), dbc.Col(graph_card2, width=6)],
            style={"width": "100%", "margin": "auto"},
        ),
        dbc.Row(
            [dbc.Col(table_card, width=12)], style={"width": "100%", "margin": "auto"}
        ),
    ]
)
...
@callback(
    ...
    [Input("select_contry", "value"), Input("select_equipment", "value")],
)

def update_total_count(selected_country, selected_equipment):
    filtered_data = df[
        (df["Страна"] == selected_country) & (df["Тип турбины"] == selected_equipment)
    ]
    ...
    scatter = px.scatter(
        filtered_data, x="Год выпуска", y="Суммарная мощность", color="Тип станции"
    )
    ...
    # create data table
    raw_data = filtered_data.drop(
        ["Unnamed: 0", "Тип турбины", "Тип станции", "Страна"], axis=1
    )
    ...
    tb = dash_table.DataTable(
        ...
        style_header={"textAlign": "center", "backgroundColor": "rgb(200, 200, 200)"},
        page_size=15,
    )
